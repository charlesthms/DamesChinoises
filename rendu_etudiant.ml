...
let deep_copy (cfg:configuration) = let (cc, lst) = cfg in
  ((cc, lst):configuration) ;;
  
let rec coup_possible_aux (cfg1:configuration) (cfg:configuration) (c1:case) = 
  let (cc, lst) = cfg in
  match cc with
    [] -> []
    | (case, color) :: t -> 
      if est_dep_unit cfg1 c1 case then (color, case) :: coup_possible_aux cfg1 (t, lst) c1
      else coup_possible_aux cfg1 (t,lst) c1
     ;;

let coup_possible (cfg:configuration) (c1:case) = coup_possible_aux cfg (deep_copy cfg) c1 ;;
...
let (test:configuration) = ([(c1, Vert); (c2, Libre)], [ Vert ; Jaune ; Rouge ; Noir ; Bleu ; Marron ]) ;;

coup_possible configuration_initial c1 ;;
est_dep_unit configuration_initial c1 c2 ;;
  ...
  let make_coup coup =
    match coup with
      Du (c1, c2) ->
          ...
          else Error "Le coup n'est pas un déplacement unitaire."
      | Sm lst ->
